...
module.exports.createCampground = async (req, res, next) => {
  ...
  campground.images = req.files.map((f) => ({
    url: f.path,
    filename: f.filename,
  })); // map over the array that is returned, add the url and filename for our model
...
module.exports.showCampground = async (req, res) => {
  const campground = await Campground.findById(req.params.id)
    .populate({
      // nested populate -- populate the reviews, then populate the author of that review
      path: "reviews",
      populate: {
        path: "author",
      },
...
module.exports.updateCampground = async (req, res) => {
  ...
  if (req.body.deleteImages) {
    ...
    }); // (db) pull from images arr, all imgs where the filename is in the req.body.deleteImages array
  }
